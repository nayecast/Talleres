...
object Problema1 extends App {
    ...
    val calificaciones = Var(List(5))

    // Signal calcula automaticamente la suma de todas las calificaciones
    val sumafinal = Signal {
      calificaciones.value.sum // Cada vez que la lista cambie esta señal se vuelve a evaluar
    }

    // Un observador que se activa cada vez que la señal sumaTotal cambia
    sumafinal.observe { nueSuma =>
      println(s"Suma actual = $nueSuma") // Imprime la nueva suma actualizada
    }

    // Agrego una nueva calificación (10) al final de la lista
    calificaciones.set(calificaciones.now :+ 10)// Por lo que la señal sumaTotal se vuelve a calcular

    // Agrego otra calificación (20)
    calificaciones.set(calificaciones.now :+ 20)

}

```
<img width="1080" height="360" alt="image" src="https://github.com/user-attachments/assets/8a77130d-3bf2-4fbe-935b-69ef3945d587" />
...
2.  **Cálculo Dinámico (`Signal`):**
    * Define una señal llamada `energiaCinetica` que calcule el valor automáticamente usando la fórmula:
    ...
    E_k = \frac{1}{2} m v^2
    $$
    <img width="135" height="65" alt="image" src="https://github.com/user-attachments/assets/f91c1a45-d785-433d-84eb-44d52fc79b00" />
...
object Problema2 extends App{
    ...
    val monitor = energiaCinetica.observe { encine =>
      println(s"Energía Cinética: $encine")
...
```
<img width="732" height="434" alt="image" src="https://github.com/user-attachments/assets/fdd87a32-9cd1-4d47-9a23-97830993c331" />
