...
La sintaxis que es mas facil para mi es python ya que esto es porque tiene una estructura s muy simple, directa que es mas facil de comprender
...
def get_average(nums):
    return reduce(lambda acc, x: acc + x, nums, 0) / len(nums)
